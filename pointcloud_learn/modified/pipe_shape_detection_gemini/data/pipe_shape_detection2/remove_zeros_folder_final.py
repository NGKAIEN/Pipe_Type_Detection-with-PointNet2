...
# 定义要处理的文件夹路径
folder_path = "./right"  # 将此路径替换为你的文件夹路径
...
for file_name in os.listdir(folder_path):
    ...
    if file_name.endswith('.ply'):
        ...
        # 重新排列数据成三维形式
        num_points = len(points)                    # 点云数x3
        total_num_points = points.shape[0]          # 获取点云的数量，即行数
        max_points = total_num_points               # 这原来是设置一个最大数以降低点云数的，为适应不同场景的点云的数量，这边设置成等于点云的数量
        padded_points = np.zeros((max_points, 3))   # 创建一个二维数组来存储数据
